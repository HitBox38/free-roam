...
export async function GET(request: Request) {
  ...
  if (!text) return Response.json({ places: [] })
  ...
  if (!response.ok) {
    return Response.json({ message: "Geoapify search failed" }, { status: 502 })
  }
  const data = await response.json()
  return Response.json({ places: normalizeGeoapifyFeatureCollection(data) })
}
